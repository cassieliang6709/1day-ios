...
final class SevenDayVisibilityUITests: XCTestCase {
    func testSevenDaySwitchShowsTemplates() {
        ...
        XCTAssertTrue(moving.waitForExistence(timeout: 5))
        ...
        app.buttons["一日"].tap()
        XCTAssertTrue(app.staticTexts.matching(NSPredicate(format: "label CONTAINS %@", "完美的一天")).firstMatch.waitForExistence(timeout: 3))
        seven.tap()
        XCTAssertTrue(moving.waitForExistence(timeout: 3))
        app.buttons.matching(NSPredicate(format: "label CONTAINS %@", "早起的人")).firstMatch.tap()
        XCTAssertTrue(app.staticTexts.matching(NSPredicate(format: "label BEGINSWITH %@", "早起的人、")).firstMatch.waitForExistence(timeout: 3))
    }
    ...
    func testDemoIsReachableFromHomeWithoutSharedRoom() {
        ...
        demo.tap()
        XCTAssertTrue(app.staticTexts["local-room-notice"].waitForExistence(timeout: 8))
        XCTAssertTrue(app.buttons["三人"].exists)
        XCTAssertTrue(app.buttons["看成片"].waitForExistence(timeout: 20))
